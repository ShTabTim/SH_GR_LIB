...
void rend(shwin* wnd) {
	hd.clear(0xFF00000F);
	for (uint32_t u(256); u--;) {
		hd.circ(rand() % hd.width, rand() % hd.height, 32 + rand() % 32, 0xFF000000 + 0x00FFFF00 & (rand()));
		hd.rect(rand() % hd.width, rand() % hd.height, 32 + rand() % 32, 32 + rand() % 32, 0xFF000000 + 0x00FFFF00 & (rand()));
		uint32_t x = rand() % hd.width;
		uint32_t y = rand() % hd.height;
		hd.rect2p(x, y, x + rand() % 64, y + rand() % 64, 0xFF000000 + 0x00FFFF00 & (rand()));
...
int WinMain(HINSTANCE hinst, HINSTANCE prev_hinst, LPSTR cmd_line, int show_cmd) {
	...
	hd.init(2*480, 2*320);
	wnd.init(hinst, L"Graphics test", rend, 2*480, 2*320);
